...
\begin{remarkbox}[title={Parameter Use and Interpretation}]
...
\midrule
$\Lambda$ & Recruitment into the susceptible population & Simulation/normalisation choice\\
$\mu$ & Natural mortality rate & Estimated or assumed\\
$\beta_0$ & Baseline transmission intensity & Literature-guided/assumed\\
$\eta$ & Relative infectiousness under treatment & Literature-guided/assumed\\
$\tau$ & Treatment initiation rate & Literature-guided/assumed\\
$\delta$ & Progression from infection to AIDS & Literature-guided/assumed\\
$\rho$ & Progression from treatment to AIDS & Literature-guided/assumed\\
...
\noindent\textbf{Proof Outline.}
At the boundary of the nonnegative region, the right-hand side of each compartment equation is nonnegative in the corresponding boundary component. Specifically, if $S=0$, then $\CaputoD{t}{q}S=\Lambda\geq0$; if $I=0$, then $\CaputoD{t}{q}I=\lambda(t)S\geq0$; if $T=0$, then $\CaputoD{t}{q}T=\tau(1+u_3)I\geq0$; and if $A=0$, then $\CaputoD{t}{q}A=\delta I+\rho(1-u_4)T\geq0$. By the positivity principle for Caputo fractional systems, solutions starting in the nonnegative orthant remain nonnegative. \hfill$\square$
...
The disease-free equilibrium is locally asymptotically stable for the fractional model if all eigenvalues $\lambda_i$ of the linearized infected subsystem at $E_0$ satisfy
\begin{equation}
|\arg(\lambda_i)|>\frac{q\pi}{2}.
\end{equation}
When $q=1$, this reduces to the classical ordinary differential equation stability condition.

For this model, $\Rzero<1$ means that the expected number of secondary infections generated by one infected individual is below the epidemic threshold. The fractional stability condition then verifies that the memory-dependent linearized system still moves away from infection persistence and toward the disease-free equilibrium.
...
As $q$ decreases below one, the memory effect becomes stronger. In epidemiological terms, this means that past infection levels, treatment history, adherence behaviour, and accumulated behavioural change may continue to influence the present trajectory. Therefore, the fractional-order comparison is useful for determining whether the classical model underestimates or overestimates the persistence of infection, treatment burden, and AIDS progression.
